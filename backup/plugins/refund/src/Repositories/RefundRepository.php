...
class RefundRepository
{
    ...
    public function updateReturnRequestStatus($request, $seller_id = null)
    {
        try {
            ...
            if ($request['return_status'] == config('ecommerce.return_request_status.pending') || $request['return_status'] == config('ecommerce.return_request_status.processing') || $request['return_status'] == config('ecommerce.return_request_status.product_received')) {
                $order_product_updated_status = config('ecommerce.product_return_status.processing');
            }
            if ($request['return_status'] == config('ecommerce.return_request_status.cancelled')) {
                $order_product_updated_status = config('ecommerce.product_return_status.return_cancel');
            }
            if ($request['return_status'] == config('ecommerce.return_request_status.approved')) {
                $order_product_updated_status = config('ecommerce.product_return_status.returned');
            }
            ...
            if ($seller_id == null && $refund_request->refund_status != $request['payment_status']) {
                ...
                if ($request['payment_status'] == config('ecommerce.return_request_payment_status.refunded')) {
                    $refund_request->total_refund_amount = $request['refund_amount'];
                }
                //payment in wallet
                if ($request['payment_status'] == config('ecommerce.return_request_payment_status.refunded') && $request->has('paid_by') && $request['paid_by'] == 'wallet' && isActivePlugin('wallet')) {
                    $wallet_transaction = new \Plugin\Wallet\Models\WalletTransaction();
                    $wallet_transaction->entry_type = config('ecommerce.wallet_entry_type.credit');
                    $wallet_transaction->recharge_type = config('ecommerce.wallet_recharge_type.refund');
    ...
    {
        if ($type == 'return') {
            if ($status == config('ecommerce.return_request_status.approved')) {
                return 'Refund request approved';
            }
            if ($status == config('ecommerce.return_request_status.pending')) {
                return 'Refund request pending';
            }
            if ($status == config('ecommerce.return_request_status.processing')) {
                return 'Refund request processing';
            }
            if ($status == config('ecommerce.return_request_status.cancelled')) {
                return 'Refund request cancelled';
            }
            if ($status == config('ecommerce.return_request_status.product_received')) {
                return 'Product received';
            }
        } else {
            if ($status == config('ecommerce.return_request_payment_status.refunded')) {
                return 'Payment refunded';
            }
            if ($status == config('ecommerce.return_request_payment_status.pending')) {
