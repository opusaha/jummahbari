                    ...
                    <form method="get" action="{{ route('plugin.refund.requests') }}">
                        <select class="theme-input-style mb-10" name="payment_status">
                            ...
                            <option value="{{ config('ecommerce.return_request_payment_status.pending') }}"
                                @selected(request()->has('payment_status') && request()->get('payment_status') == config('ecommerce.return_request_payment_status.pending'))>
                                {{ translate('Pending') }}
                            </option>
                            <option value="{{ config('ecommerce.return_request_payment_status.refunded') }}"
                                @selected(request()->has('payment_status') && request()->get('payment_status') == config('ecommerce.return_request_payment_status.refunded'))>
                                {{ translate('Refunded') }}
                            </option>
                        ...
                        <select class="theme-input-style mb-10" name="return_status">
                            ...
                            <option value="{{ config('ecommerce.return_request_status.pending') }}"
                                @selected(request()->has('return_status') && request()->get('return_status') == config('ecommerce.return_request_status.pending'))>
                                {{ translate('Pending') }}
                            </option>
                            <option value="{{ config('ecommerce.return_request_status.processing') }}"
                                @selected(request()->has('return_status') && request()->get('return_status') == config('ecommerce.return_request_status.processing'))>
                                {{ translate('Processing') }}
                            </option>
                            <option value="{{ config('ecommerce.return_request_status.product_received') }}"
                                @selected(request()->has('return_status') && request()->get('return_status') == config('ecommerce.return_request_status.product_received'))>
                                {{ translate('Product Received') }}
                            </option>
                            <option value="{{ config('ecommerce.return_request_status.approved') }}"
                                @selected(request()->has('return_status') && request()->get('return_status') == config('ecommerce.return_request_status.approved'))>
                                {{ translate('Approved') }}
                            </option>
                            <option value="{{ config('ecommerce.return_request_status.cancelled') }}"
                                @selected(request()->has('return_status') && request()->get('return_status') == config('ecommerce.return_request_status.cancelled'))>
                                {{ translate('Cancelled') }}
                            </option>
                        </select>
                            ...
                            @if ($refund_request_list->count() > 0)
                                @foreach ($refund_request_list as $key => $request)
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href="{{ route('plugin.ecommerce.orders.details', ['id' => $request->order_id]) }}">
                                                {{ $request->order_code }}
                                        ...
                                        <td>
                                            <a
                                                href="{{ route('plugin.ecommerce.customers.details', ['id' => $request->customer_id]) }}">
                                        ...
                                        <td>
                                            ...
                                            @elseif ($request->return_status == config('ecommerce.return_request_status.processing'))
                                                <p class="badge badge-primary">{{ translate('Processing') }}</p>
                                            @elseif ($request->return_status == config('ecommerce.return_request_status.cancelled'))
                                                <p class="badge badge-danger">{{ translate('Cancelled') }}</p>
                                            @elseif ($request->return_status == config('ecommerce.return_request_status.product_received'))
