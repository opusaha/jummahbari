...
class CustomerRepository
{
    ...
    public function customerRegister($request)
    {
        try {
            DB::beginTransaction();
            $status = SettingsRepository::getEcommerceSetting('customer_auto_approved') == config('settings.general_status.active') && SettingsRepository::getEcommerceSetting('customer_email_verification') == config('settings.general_status.in_active') ? config('settings.general_status.active') : config('settings.general_status.in_active');
            ...
            if (SettingsRepository::getEcommerceSetting('customer_email_verification') == config('settings.general_status.active')) {
                ...
                $mail_data = [
                    ...
                    'subject' => get_setting('site_title') . ' Email Verification',
                    '_system_name_' =>  get_setting('site_title'),
     ...
     * @return Collections
     */
    public function customerAllAddress($customer_id, $status = null)
    {
        try {
            $query = CustomerAddress::with(['country', 'state', 'city'])
