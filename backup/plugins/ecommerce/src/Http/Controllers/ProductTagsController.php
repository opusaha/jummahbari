...
use Plugin\Ecommerce\Http\Requests\ProductTagsRequest;
...
class ProductTagsController extends Controller
{
    ...
    {
        return view('plugin/ecommerce::products.tags.index')->with(
            [
    ...
    public function storeTag(ProductTagsRequest $request)
    {
        ...
        if ($res == true) {
            ...
            return redirect()->route('plugin.ecommerce.product.tags.list');
        } else {
    ...
    {
        return view('plugin/ecommerce::products.tags.edit_tag')->with(
            [
