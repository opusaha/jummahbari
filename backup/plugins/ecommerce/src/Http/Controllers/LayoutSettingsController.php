...
class LayoutSettingsController extends Controller
{
    ...
    {
        $this->menus = [];
        $data = getMenuStructure(getMenuPositionId('Main Header Menu'));
    ...
    public function getFooterWidgets(Request $request)
    {
        try {
            ...
            $sidebar_id = getThemeSidebarId('site_footer');
            ...
            foreach ($all_blog_sidebar_widgets as $key => $widget) {
                ...
                if ($widget_key == 'recent_blog_widget') {
                    $widget_values = getSidebarWidgetValues($sidebar_has_widget, Session::get('api_locale'));
                ...
                //footer menu
                if ($widget_key == 'footer_menu') {
                    $widget_values = getSidebarWidgetValues($sidebar_has_widget, Session::get('api_locale'));
                    $footer_menu = $widget_values;
                    $footer_menu_array = (array)$footer_menu;

                    if (sizeof($footer_menu_array) > 0) {
                        $footer_menu_items = $this->footerWidgetLeftMenus($footer_menu_array['menu_group_id']);
                        $widget_options[$key] = [
                            'widget' => $widget_key,
                            'widget_title' => $footer_menu_array['widget_title'],
                ...
                if ($widget_key == 'site_info_widget') {
                    $contact_info =  getSidebarWidgetValues($sidebar_has_widget, Session::get('api_locale'));
                    $contact_info['play_store_icon'] = isset($contact_info['play_store_icon']) ? asset(getFilePath($contact_info['play_store_icon'])) : '';
                    $contact_info['app_store_icon'] = isset($contact_info['app_store_icon']) ? asset(getFilePath($contact_info['app_store_icon'])) : '';

                    $widget_options[$key] = [
                        'widget' => $widget_key,
                        'contact_info' => $contact_info,
                        'social_links' =>  $this->getSocialIcons()
                    ];
                }

                //Newsletter widget
                if ($widget_key == 'newsletter_widget') {
                    $newsletter_info =  getSidebarWidgetValues($sidebar_has_widget, Session::get('api_locale'));
                    $widget_options[$key] = [
                        'widget' => $widget_key,
