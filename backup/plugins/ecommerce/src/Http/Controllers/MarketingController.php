...
use Plugin\Ecommerce\Notifications\CustomNotification;
use Plugin\Ecommerce\Repositories\MarketingRepository;
use Plugin\Ecommerce\Notifications\CustomMailNotification;
use Plugin\Ecommerce\Http\Requests\CustomNotificationRequest;
...
class MarketingController extends Controller
{
    ...
    public function customNotifications(Request $request)
    {
        ...
        return view('plugin/ecommerce::marketing.custom_notifications')->with(
            [
    ...
    {
        return view('plugin/ecommerce::marketing.new_custom_notifications');
    }
    ...
    public function sendCustomNotification(CustomNotificationRequest $request)
    {
        try {
            DB::beginTransaction();
            $receivers = [];
            if ($request['send_to'] == config('ecommerce.custom_notification_receiver_type.all_customers')) {
                $receivers = Customers::where('status', config('settings.general_status.active'))->get();
            }

            if ($request['send_to'] == config('ecommerce.custom_notification_receiver_type.specific_customer')) {
                $receivers = Customers::where('status', config('settings.general_status.active'))->whereIn('id', $request['customers'])->get();
            }

            if ($request['send_to'] == config('ecommerce.custom_notification_receiver_type.all_users')) {
                $receivers = User::where('status', config('settings.general_status.active'))->where('id', '!=', auth()->user()->id)->get();
            }

            if ($request['send_to'] == config('ecommerce.custom_notification_receiver_type.specific_user')) {
                $receivers = User::where('status', config('settings.general_status.active'))->whereIn('id', $request['users'])->get();
            }
            if ($request['send_to'] == config('ecommerce.custom_notification_receiver_type.specific_user_role')) {
            ...
            if ($request['notification_type'] == config('ecommerce.custom_notification_type.dashboard')) {
                $data = [
                    'link' => "#",
            ...
            if ($request['notification_type'] == config('ecommerce.custom_notification_type.email')) {
                $data = [
            ...
            if ($request['notification_type'] == config('ecommerce.custom_notification_type.email_dashboard')) {
                //Send Database notification
                $data = [
                    'link' => "#",
                    'message' => $request['message']
            ...
            return to_route('plugin.ecommerce.marketing.custom.notification.create.new');
        } catch (\Exception $e) {
            DB::rollBack();
