...
use Plugin\Ecommerce\Models\ProductCollection as ProductCollectionModel;
...
class ProductController extends Controller
{
    ...
    public function collectionOption(Request $request)
    {
        $query = ProductCollectionModel::select('id', 'name')
        ...
        $items = $query->orderBy('id', 'asc')->paginate(2);

        $output = [];

        foreach ($items->items() as $value) {
            $item['id'] = $value->id;
            $item['text'] = $value->translation('name', getLocale());
            array_push($output, $item);
        }

        $morePages = true;

        if (empty($items->nextPageUrl())) {
            $morePages = false;
        }
        $results = array(
            "results" => $output,
    ...
    public function flashDealOption(Request $request)
    {
        $query = FlashDeal::select('id', 'title')
            ->where('status', config('settings.general_status.active'));
        ...
        foreach ($items->items() as $value) {
            $item['id'] = $value->id;
            $item['text'] = $value->translation('title', getLocale());
            array_push($output, $item);
        }
