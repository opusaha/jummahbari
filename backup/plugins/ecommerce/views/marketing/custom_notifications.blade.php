            ...
            <div class="card mb-30">
                <div class="card-body border-bottom2 mb-20">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('plugin.ecommerce.marketing.custom.notification.create.new') }}"
                            ...
                            @if ($notifications->count() > 0)
                                @foreach ($notifications as $key => $notification)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($notification->to == config('ecommerce.custom_notification_receiver_type.all_customers'))
                                                {{ translate('All Customers') }}
                                            @endif
                                            @if ($notification->to == config('ecommerce.custom_notification_receiver_type.specific_customer'))
                                                {{ translate('Specific Customers') }}
                                            @endif
                                            @if ($notification->to == config('ecommerce.custom_notification_receiver_type.all_users'))
                                                {{ translate('All Users') }}
                                            @endif
                                            @if ($notification->to == config('ecommerce.custom_notification_receiver_type.specific_user'))
                                                {{ translate('Specific Users') }}
                                            @endif
                                            @if ($notification->to == config('ecommerce.custom_notification_receiver_type.specific_user_role'))
                                                {{ translate('Specific User Roles') }}
                                            @endif
                                        </td>
                                        <td>
                                            @if ($notification->type == config('ecommerce.custom_notification_type.dashboard'))
                                                {{ translate('Dashbaord') }}
                                            @endif
                                            @if ($notification->type == config('ecommerce.custom_notification_type.email'))
                                                {{ translate('Email') }}
                                            @endif
                                            @if ($notification->type == config('ecommerce.custom_notification_type.email_dashboard'))
            ...
            $('.fire-bulk-action').on('click', function(e) {
                ...
                if (action != "") {
                    ...
                    if (selected_items.length > 0) {
                        $.post('{{ route('plugin.ecommerce.marketing.custom.notification.bulk.action') }}', {
                            _token: '{{ csrf_token() }}',
                            data: data
                        }, function(data) {
                            if (data.success) {
