 @foreach ($countries as $country)
     <li class="cl-item cl-item-open">
         ...
         <div class="cl-label-wrap">
             @if (\Plugin\Ecommerce\Repositories\ShippingRepository::countryInAnotherShippingRate($country->id, $rate_id))
                 @php
                     $country_rates = \Plugin\Ecommerce\Models\LocationWiseShippingRateCountry::where(
                         'country_id',
                         $country->id,
                     )
                         ->pluck('rate_id')
         ...
         @if (count($states) > 0)
             <ul class="js-check-all-target js-check-all" data-check-all="country-{{ $country->id }}">
                 @foreach ($states as $state)
                     ...
                     <li class="cl-item">
                         <div class="cl-label-wrap">
                             @if (\Plugin\Ecommerce\Repositories\ShippingRepository::stateInAnotherShippingRate($state->id, $rate_id))
                                 @php
                                     $state_rates = \Plugin\Ecommerce\Models\LocationWiseShippingRateStates::where(
                                         'state_id',
                                         $state->id,
                                     )
                                         ->pluck('rate_id')
                                 ...
                                 @foreach ($cities as $city)
                                     <li class="cl-item">
                                         <div class="cl-label-wrap">
                                             ...
                                             @if (\Plugin\Ecommerce\Repositories\ShippingRepository::cityInAnotherShippingRate($city->id, $rate_id))
                                                 @php
                                                     $city_rates = \Plugin\Ecommerce\Models\LocationWiseShippingRateCity::where(
                                                         'city_id',
                                                         $city->id,
                                                     )
                                                         ->pluck('rate_id')
