 @foreach ($countries as $country)
     <li class="cl-item cl-item-open">
         @php
             $states = \Plugin\Ecommerce\Models\States::with(['state_translations'])
                 ->where('country_id', $country->id)
                 ->get();
         @endphp
         <div class="cl-label-wrap">
             @if (\Plugin\Ecommerce\Repositories\ShippingRepository::countryInAnotherZone($country->id, $profile_id))
                 @php
                     $country_zones = \Plugin\Ecommerce\Models\ShippingZoneCountries::where('country_id', $country->id)
                         ->pluck('zone_id')
                         ->toArray();
                 @endphp
         ...
         @if (count($states) > 0)
             <ul class="js-check-all-target js-check-all" data-check-all="country-{{ $country->id }}">
                 @foreach ($states as $state)
                     @php
                         $cities = \Plugin\Ecommerce\Models\Cities::with(['city_translations'])
                             ->where('state_id', $state->id)
                             ->get();
                     @endphp
                     <li class="cl-item">
                         <div class="cl-label-wrap">
                             @if (\Plugin\Ecommerce\Repositories\ShippingRepository::stateInAnotherZone($state->id, $profile_id))
                                 @php
                                     $state_zones = \Plugin\Ecommerce\Models\ShippingZoneStates::where('state_id', $state->id)
                                         ->pluck('zone_id')
                                         ->toArray();
                                 @endphp
                                 ...
                                 @foreach ($cities as $city)
                                     <li class="cl-item">
                                         <div class="cl-label-wrap">
                                             ...
                                             @if (\Plugin\Ecommerce\Repositories\ShippingRepository::cityInAnotherZone($city->id, $profile_id))
                                                 @php
                                                     $city_zones = \Plugin\Ecommerce\Models\ShippingZoneCities::where('city_id', $city->id)
                                                         ->pluck('zone_id')
                                                         ->toArray();
                                                 @endphp
