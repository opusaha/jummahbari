...
use Plugin\Flashdeal\Http\Resources\DealResource;
use Plugin\Flashdeal\Http\Resources\DealCollection;
use Plugin\Flashdeal\Http\Requests\FlashDealRequest;
use Plugin\Flashdeal\Repositories\FlashDealRepository;
use Plugin\Flashdeal\Http\Resources\DealProductCollection;
...
class FlashDealController extends Controller
{
    ...
    public function activeFlashDeals(Request $request)
    {
        $deals = $this->flashdeal_repository->active_flash_deals();

        return new DealCollection($deals);
    }

    public function fashDealDetails(Request $request)
    {

        $request->validate([
            'id' => 'required'
        ]);
        $deal = FlashDeal::find($request->id);
    ...
    public function fashDealProducts(Request $request)
    {
        $request->validate([
            'id' => 'required'
        ]);

        $product_query = FlashDealProducts::with(['product' => function ($q) {
            $q->with(['single_price', 'variations']);
        }])->where('deal_id', $request->id);

        if ($request->has('perPage')) {
            $products = $product_query->paginate($request->perPage);
        } else {
        ...
        return new DealProductCollection($products);
    }
}
