...
class DashboardController extends Controller
{
    ...
    public function dashboard()
    {
        $pending_earnings = SellerEarnings::where('seller_id', auth()->user()->id)
            ->where('status', config('ecommerce.seller_earning_status.pending'))
        ...
        if ($pending_earnings != null) {
            foreach ($pending_earnings as $earning) {
                ...
                if ($order_product_details != null) {
                    if ($order_product_details->canReturn() != config('settings.general_status.active') && $order_product_details->return_status != config('ecommerce.product_return_status.returned') && $order_product_details->return_status != config('ecommerce.product_return_status.processing')) {
                        $updatable_earning = SellerEarnings::where('id', $earning->id)->first();
                        $updatable_earning->status = config('ecommerce.seller_earning_status.approve');
