...
<script>
    ...
    (function($) {
        ...
        $(document).ready(function() {
            ...
            $('.widget-single').draggable({
                ...
                start: function(event, ui) {
                    ...
                    ui.helper.find(".widget-content").css("border", "none");
                    ui.helper.find(".widget-content").find('i').remove();
                }
            });

            // Sections sortable initialize
            ...
            // Widget Droppable and Sortable initialize
        ...
        function loadWidgetList() {
            $('#properties-section').find('h4').html("{{ translate('Elements') }}");
            $(".single-section").removeClass('bg-primary-light');
            $(".single-widget").removeClass('bg-primary-light');
        ...
        function appendSectionToPage(layout, section_id, layout_ids) {
            ...
            for (let i = 0; i < columns.length; i++) {
                columns_markup +=
                    `<div class="col-${columns[i]} p-0 section-column" style="border:1px solid" data-section-layout-id="${layout_ids[i]}"></div>`;
            }
            let layout_markup = `
        ...
        // Will initialize the droppable and sortable of widget
        function droppableAndSortableInit() {
            // Section column droppable for widgets initialize
            ...
            // Section columns are sortable initialize
        ...
        function appendWidgetToLayout(widget, section, id) {
            $(widget).find('.widget-content').find('i').remove();
            $(widget).removeClass(
                'mb-2 text-center widget-single ui-draggable ui-draggable-handle col-lg-6 d-flex justify-content-between align-items-center'
            );
            $(widget).addClass('section-widget single-widget');

            $(widget).find('.widget-content').addClass(
                'card card-body flex-row bg-transparent justify-content-between px-3 py-3 flex-wrap gap-10');
        ...
        // Disable All Fields That Are Not For Translate
        function disableNotTranslatedField() {
        ...
        function getWidgetProperties(widget, layout_widget_id, section_id, lang) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // Disable Fields if not default language
                    if (lang != "{{ getDefaultLang() }}") {
                        disableNotTranslatedField();
                    }
