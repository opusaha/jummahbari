...
class LanguageController extends Controller
{
    ...
    public function themeTranslations()
    {
        ...
        if ($active_theme != null) {
            ...
            $lang_keys = ThemeTranslations::where('lang', 'en')->where('theme', $active_theme->location)->orderBy('id', 'DESC')
    ...
    {
        try {
            $lang_key = $request['value'];
            $active_theme = getActiveTheme();
            $translation_def = ThemeTranslations::firstOrCreate(['lang_key' => $lang_key, 'theme' => $active_theme->location, 'lang' => 'en']);
            $translation_def->lang_value = xss_clean($request['value']);
            $translation_def->save();
            toastNotification('success', translate('Translation added successfully'), 'Success');
            return redirect()->route('core.language.theme.translations');
        } catch (\Exception $e) {
            toastNotification('error', translate('Translation adding failed'), 'Failed');
            return redirect()->back();
        }
    }

    public function deleteThemeTranslations(Request $request)
    {
        try {
            $id = $request->id;
            ThemeTranslations::where('id', $id)->delete();
            toastNotification('success', translate('Translation deleted successfully'), 'Success');
            return redirect()->route('core.language.theme.translations');
        } catch (\Exception $e) {
            toastNotification('error', translate('Translation deleting failed'), 'Failed');
            return redirect()->back();
        }
    }
    /**
