...
class MediaController extends Controller
{
    ...
    public function mediaSettings()
    {
        ...
        return view('core::base.business.media.media_settings');
    }
    ...
    public function storeMediaSettings(MediaRequest $request)
    {
        try {
            ...
            foreach ($request->except(['watermark_status', '_token', 'files', 'image_applicable_folder', 'aws_access_key_id', 'aws_secret_access_key', 'aws_default_region', 'aws_bucket', 'aws_endpoint']) as $key => $value) {
                set_setting($key, $value);
            }

            $watermark_status = $request->has('watermark_status') ? 'on' : 'off';
            set_setting('watermark_status', $watermark_status);
