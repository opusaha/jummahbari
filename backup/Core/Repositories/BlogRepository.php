...
class BlogRepository
{
    ...
    public function getBlogComment($permalink)
    {
        ...
        if (get_setting('page_comments') == 1) {
            $paginate = get_setting('comments_per_page');
            $order = get_setting('comments_order') == 1 ? 'DESC' : 'ASC';
        } else {
            $paginate = null;
            $order = 'DESC';
        }
    ...
    public function updateCommentSetting($request)
    {
        ...
        try {
            $comment_settings = [];
            $comment_settings['close_comments_days_old'] = $request['close_comments_days_old'] == null ? 1 : $request['close_comments_days_old'];
            $comment_settings['comments_per_page'] = $request['comments_per_page'] == null ? 8 : $request['comments_per_page'];
            $comment_settings['default_comment_status'] = $request->has('default_comment_status') ? 1 : 2;
            $comment_settings['require_name_email'] = $request->has('require_name_email') ? 1 : 2;
            $comment_settings['comment_registration'] = $request->has('comment_registration') ? 1 : 2;
            $comment_settings['close_comments_for_old_blogs'] = $request->has('close_comments_for_old_blogs') ? 1 : 2;
            $comment_settings['thread_comments'] = $request->has('thread_comments') ? 1 : 2;
            $comment_settings['page_comments'] = $request->has('page_comments') ? 1 : 2;
            $comment_settings['comments_notify_email'] = $request->has('comments_notify_email') ? 1 : 2;
            $comment_settings['comments_moderation_notify_email'] = $request->has('comments_moderation_notify_email') ? 1 : 2;
            $comment_settings['comment_moderation'] = $request->has('comment_moderation') ? 1 : 2;
            $comment_settings['comment_previously_approved'] = $request->has('comment_previously_approved') ? 1 : 2;
            $comment_settings['show_avatars'] = $request->has('show_avatars') ? 1 : 2;
            $comment_settings['comment_max_links'] = $request['comment_max_links'];
            $comment_settings['comment_moderation_keys'] = $request['comment_moderation_keys'];
            $comment_settings['comment_disallowed_keys'] = $request['comment_disallowed_keys'];
            $comment_settings['avatar_default'] = isset($request['avatar_default']) ? $request['avatar_default'] : 'mystery';
            $comment_settings['thread_comments_level'] = $request['thread_comments_level'];
            $comment_settings['comment_order'] = $request['comment_order'];
            DB::beginTransaction();
            foreach ($comment_settings as $key => $value) {
    ...
    public function commentFiltration($comment)
    {
        ...
        if (get_setting('comment_disallowed_keys') != null) {
            if ($this->dissAllowedKeys($comment, get_setting('comment_disallowed_keys'))) {
                return ['error' => translate('Your Comment is not Approved')];
            }
        }

        // filter for links in comment
        if ($this->commentMaxLink($comment, get_setting('comment_max_links'))) {
            return ['warning' => translate('Your Comment is in Moderation')];
        }

        // filter for comment moderation
        if (get_setting('comment_moderation_keys') != null) {
            if ($this->commentModeration($comment, get_setting('comment_moderation_keys'))) {
                return ['warning' => translate('Your Comment is in Moderation')];
            }
        }

        // checking if comment manual is on and previously approve or not
        if (get_setting('comment_moderation') == 1) {
            if (get_setting('comment_previously_approved') == 1) {
        ...
        if (get_setting('comments_notify_email') == 1) {
    ...
    private function commentMaxLink($comment, $max_links)
    {
        ...
        if ($max_links == 0 || $max_links == null) {
            ...
            if ($link_count >=  $max_links) {
                ...
                $updated_comment->blog_name = $comment['blog_name'];
                $updated_comment->blog_permalink = $comment['blog_permalink'];
                if (get_setting('comments_notify_email') == 1 || get_setting('comments_moderation_notify_email') == 1) {
                    $this->sendMail($updated_comment);
                }
                return true;
            } else {
    ...
    public function sendMail($comment)
    {
        ...
        if (get_setting('comments_moderation_notify_email') == 1) {
