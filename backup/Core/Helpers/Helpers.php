...
if (!function_exists('get_setting')) {
    ...
    {
        $settings = Cache::rememberForever('general-settings', function () {
            return GeneralSettings::pluck('value', 'key')->toArray();
        });

        if (!isset($settings[$key])) {
            $setting_def =  GeneralSettings::firstOrCreate(['key' => $key]);
            $setting_def->key = $key;
            $setting_def->value = $default;
            $setting_def->save();
...
if (!function_exists('set_setting')) {
    function set_setting($settings_name, $value = NULL)
    {
        try {
            DB::beginTransaction();
            $setting = GeneralSettings::firstOrCreate(['key' => $settings_name]);
            $setting->value = $value;
            $setting->save();
            DB::commit();
            cache_clear();
...
if (!function_exists('getEmailTemplateOf')) {
    ...
    function getEmailTemplateOf($template_id, $data, $keywords)
    {
        ...
        if (!array_key_exists('_system_logo_url_', $data)) {
            $system_logo = asset(getFilePath(get_setting('admin_logo'), true));
...
if (!function_exists('getDefaultLang')) {
    ...
    function getDefaultLang($id = false)
    {
        $lang_id = get_setting('default_language');
...
if (!function_exists('defaultLanguage')) {
    ...
    {
        $default_language_id = get_setting('default_language');
