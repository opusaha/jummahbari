 ...
 */
   ...
   * Store cart items
   */
  storeCartItems(state, { item, messages }) {
    ...
    if (productInCart) {
      ...
      if (productInCart.quantity <= productInCart.max_item) {
        ...
        $toast.success(messages.success + '<a href="/cart">' + messages.viewCart + "</a>");
      } else {
        productInCart.quantity -= item.quantity;
  ...
  storeIsActiveBillToDifferentAddress(state, data) {
    state.billToDifferentAddress = data;
    localStorage.setItem("billToDifferentAddress", JSON.stringify(state.billToDifferentAddress));
  },
  /**
  ...
  setFinalSubtotal(state, data) {
    state.finalCartSubtotal = data;
    localStorage.setItem("finalCartSubtotal", JSON.stringify(state.finalCartSubtotal));
  },
  /**
  * Will set final tax
  */
  ...
  flushCartData(state) {
    ...
    console.log(state.checkoutItems.length, state.cart.length);
    ...
    state.billToDifferentAddress = false;
    localStorage.removeItem("billToDifferentAddress");
