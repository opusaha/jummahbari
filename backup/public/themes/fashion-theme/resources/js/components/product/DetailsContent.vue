<template>
  ...
  <div class="product-details-content py-20">
    <!--Flash deal info-->
              ...
              <label class="custom-checkbox--two position-relative" :for="`${attr.title}-option-${j + 1}`">
                ...
                <template v-if="attr.title == 'color'">
                  <span class="checkmark p-0" :style="{
                    ...
                    <img :src="option.image" :alt="`${option.name}`" v-if="option.image" />
...
export default {
  ...
  methods: {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.success) {
            ...
            this.attachment.title = this.product.attatchment_title;
            this.$toast.success(this.product.attatchment_title + " " + this.$t("upload successfully"));
          } else {
            this.$toast.error(this.product.attatchment_title + " " + this.$t("upload failed"));
          }
        })
        .catch((error) => {
     ...
     */
    placeOrder() {
      if (this.product.attatchment_title != null && this.attachment == null) {
        this.$toast.error("Please upload" + " " + this.product.attatchment_title);
        return;
      }
      ...
      this.$store.dispatch("addToCart", { item: cart_item, messages }).then(() => {
    ...
    addToCart() {
      ...
      this.$store.dispatch("addToCart", { item: cart_item, messages });
    },
    /**
     * Add to wishlist
     */
