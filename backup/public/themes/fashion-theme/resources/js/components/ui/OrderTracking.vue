<template>
  <div class="order-status-details">
    <div v-for="(item, index) in items" :key="index" class="mb-1">
      <div class="details-item mb-0 d-flex justify-content-between" v-if="max_view >= index + 1">
        <div class="d-flex flex-wrap">
          <div class="time mr-10 text-black" v-if="item.date">{{ item.date }}</div>
          <div class="time mr-10 text-black" v-else>{{ item.created_at }}</div>
