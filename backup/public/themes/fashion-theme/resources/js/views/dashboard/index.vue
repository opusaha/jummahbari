  ...
  <Dashboard>
    <div class="dashboard">
      <template v-if="!loading">
        ...
        <div class="row">
          <div class="col-lg-4">
            <dashboard-card title="Total Order" :counter="dashboard_content.total_order"></dashboard-card>
          </div>
          <div class="col-lg-4">
            <dashboard-card title="Pending Orders" :counter="dashboard_content.total_pending_order"></dashboard-card>
          </div>
          <div class="col-lg-4">
            <dashboard-card title="Total Purchase Amount" :counter="dashboard_content.total_purchase_amount"
              currency></dashboard-card>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-12">
            <h5 class="order-summery-title">{{ $t("Latest Orders") }}</h5>
            <div class="table-responsive" v-if="latestPurchases.length > 0">
              <!-- Table -->
              <CTable class="style1 mb-0" :class="latestPurchases.length == 1 ? 't-mh-160' : ''">
                <CTableHead>
                  <CTableRow>
                    ...
                    <CTableHeaderCell>{{ $t("Num of Products") }}</CTableHeaderCell>
                    <CTableHeaderCell>{{ $t("Actions") }}</CTableHeaderCell>
                  </CTableRow>
                </CTableHead>
                <CTableBody>
                  <CTableRow v-for="(tdata, index) in latestPurchases" :key="tdata.id">
                    ...
                    <CTableDataCell><the-currency :amount="tdata.total_payable_amount"></the-currency>
                    </CTableDataCell>
                    ...
                    <CTableDataCell>
                      <CDropdown>
                        ...
                        <CDropdownMenu>
                          <CDropdownItem>
                            <router-link :to="`/dashboard/order-details/${tdata.id}`">{{ $t("Details") }}</router-link>
                          </CDropdownItem>
                        </CDropdownMenu>
                      </CDropdown>
                    </CTableDataCell>
