        ...
        <div class="row">
          <div :class="blogsListLayoutClass">
            ...
            <article class="post-details" v-if="!blog_details_loading">
              ...
              <header class="entry-header">
                ...
                ">
                  ...
                  <ul class="list-inline list-unstyled entry-meta d-flex align-items-center" v-if="
                    singleBlogPageStyle.date == 1 ||
                    singleBlogPageStyle.author == 1
                  ">
                    <li v-if="singleBlogPageStyle.date == 1">
                      <span>{{ $t("Posted On:") }}</span>
                      <a :href="'/blog/' + blog.permalink"><time class="entry-date"
                          datetime="2018-08-26T05:00:16+00:00">{{ blog.publish_at }}</time></a>
                    </li>
                ...
                <template v-else>
                  ...
                  <ul class="list-inline list-unstyled entry-meta d-flex align-items-center">
                    <li v-if="singleBlogPageStyle.date == 1">
                      <span>{{ $t("Posted On:") }}</span>
                      <a :href="'/blog/' + blog.permalink"><time class="entry-date"
                          datetime="2018-08-26T05:00:16+00:00">{{ blog.publish_at }}</time></a>
              ...
              <footer class="entry-footer">
                <div class="row border-top border-bottom pt-3 pb-3 mx-0">
                  <div class="col-lg-6">
                    <!-- Tags -->
                    <div class="entry-tags">
                      <ul class="align-items-center d-flex gap-2 list-inline mb-0">
                        <li class="mr-0">{{ $t("Tags:") }}</li>
                        <template v-if="blog.tag != null">
                          <li v-for="(tag, index) in blog.tag_list" :key="index" class="mt-half">
                            <a class="text-capitalize" :href="'/blog/tag/' + tag.permalink">
                              {{ tag.name }}
                              <span v-if="index < blog.tag_list.length - 1">,</span>
                            </a>
                          </li>
                        </template>
                    ...
                    <div class="entry-categories">
                      <ul class="align-items-center d-flex gap-2 list-inline mb-0">
                        ...
                        <template v-if="blog.category != null">
                          <li v-for="(cat, index) in blog.category_list" :key="index" class="mt-half">
                            <a class="text-capitalize" :href="'/blog/category/' + cat.permalink">
                              {{ cat.name }}
                              <span v-if="index < blog.category_list.length - 1">,</span>
                            </a>
                          </li>
                        </template>
                        <li class="media-body" v-else>
            ...
            <div class="related-post pt-5">
              ...
              <div class="row">
                <div v-for="(blog, index) in blogs" :key="`blog-${index}`" class="col-md-4 col-6">
                  {{ blogs.title }}
                  <blog-card :blog="blog" :blogOptionStyle="blogOptionStyle" styleTwo class="mb-30" />
                </div>
              </div>
            </div>
            <!-- /Related Post -->
            ...
            <!--Blog Comments-->
            <div class="comments-area pt-4" id="comment-list-01" v-if="commentList">
              <div>
                <v-runtime-template :template="commentList"></v-runtime-template>
              </div>
              <!--Comment Pagination-->
              <div class="row align-items-center mt-10" v-if="endingPage > 1">
                <div class="col-md-12" v-if="commentSettings.page_comments == 1">
                  <ul class="tl-pagination custom-pagination" v-if="commentList != ''">
                    <li v-if="currentPage == 1">
                      <a class="" href="#" aria-disabled="true">«</a>
                    </li>
                    <li v-else>
                      <a class="" href="#" @click="paginate('prev', 'comment-list-01')">«</a>
                    </li>

                    <template v-if="currentPage == 1">
                      <li>
                        <a href="#" @click="paginate(currentPage, 'comment-list-01')" class="active">{{ currentPage
                        }}</a>
                      </li>
                      <li>
                        <a href="#" v-if="endingPage > 1" @click="paginate(2, 'comment-list-01')">2</a>
                      </li>
                      <li>
                        <a href="#" v-if="endingPage > 2" @click="paginate(3, 'comment-list-01')">3</a>
                      </li>
                    </template>

                    <template v-else-if="currentPage == endingPage">
                      <li>
                        <a href="#" v-if="endingPage > 1" @click="paginate(1, 'comment-list-01')">1</a>
                      </li>
                      <li>
                        <a href="#" v-if="endingPage > 2" @click="paginate(2, 'comment-list-01')">2</a>
                      </li>
                      <li>
                        <a href="#" @click="paginate(currentPage, 'comment-list-01')" class="active">{{ currentPage
                        }}</a>
                    ...
                    <template v-else>
                      <li>
                        <a href="#" v-if="endingPage > 1" @click="paginate(currentPage - 1, 'comment-list-01')">{{
                          currentPage - 1 }}</a>
                      </li>
                      <li>
                        <a href="#" @click="paginate(currentPage, 'comment-list-04')" class="active">{{ currentPage
                        }}</a>
                      </li>
                      <li>
                        <a href="#" v-if="endingPage > 2" @click="paginate(currentPage + 1, 'comment-list-01')">{{
                          currentPage + 1 }}</a>
                      </li>
                    </template>
                    ...
                    <li v-else>
                      <a href="#" @click="paginate('next', 'comment-list-01')">»</a>
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <div class="comment-respond pt-4" v-if="
              ...
              ">
                <div v-if="parent != ''">
                  ...
                  <h3>
                    {{ $t("Leave A Reply To") }}
                    {{ replayComment }}
                    <a href="javascript:void(0)" @click="cancelReplay()" class="btn-link fz-14 text-danger">
                      {{ $t("Cancel") }}</a>
                  </h3>
                </div>
                <h3 class="default-title" v-else>
                  {{ $t("Leave Your Thought") }}
                </h3>
                <div class="comment-form" ref="comment">
                  <div class="row">
                    <div v-if="!(isCustomerLogin || isAdminLogin)">
                      ...
                      <div class="col-lg-6">
                        <input class="theme-input-style" type="text" v-bind:placeholder="$t('Your Name')"
                          required="required" v-model="userName" />
                        <span v-if="errors" class="text-danger error">{{ errors.user_name }}</span>
                      </div>
                      <div class="col-lg-6">
                        <input class="theme-input-style" type="email" v-bind:placeholder="$t('Your Email')"
                          required="required" v-model="userEmail" />
                        <span v-if="errors" class="text-danger error">{{ errors.user_email }}</span>
                      </div>
                      <div class="col-lg-12">
                        <input class="theme-input-style" type="text" v-bind:placeholder="$t('Website')"
                          v-model="websiteName" />
                      </div>
                    </div>

                    <div class="col-12">
                      <textarea class="theme-input-style" required="required" v-bind:placeholder="$t('Message here')"
                        v-model="comment"></textarea>
                      <span v-if="errors" class="text-danger error">{{ errors.comment }}</span>
                    </div>
                  </div>

                  <button type="button" class="btn btn-fill" @click="postComment()">
                    {{ $t("Submit") }}
                  </button>
                </div>
              </div>
          ...
          <div class="col-lg-3" v-if="singleBlogPageStyle.blog_layout != 'full_layout'">
