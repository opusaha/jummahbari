        ...
        <div class="row" v-if="success">
                ...
                <CTable class="compare-table border-bottom-0 mb-0">
                  <CTableBody>
                    <CTableRow v-for="tdata in tableData" :key="tdata.id">
                      <CTableHeaderCell colspan="2">{{ tdata.title }}</CTableHeaderCell>
                      <CTableDataCell v-for="data in tdata.data" :key="data.name">
                        <div>
                          <span v-if="data.search" class="mb-20 d-block position-relative">
                            <div class="align-items-center btn-wrapper d-flex">
                              <input v-bind:placeholder="$t('Search Product')" class="theme-input-style"
                                v-model="data.search_key" v-on:keyup="searchItemsFromList(data.id)" />
                              <button @click.prevent="removeItem(data.id)"
                                class="align-items-center d-flex ml-10 remove-btn">
                                <span class="material-icons">delete</span>
                              </button>
                            </div>

                            <div class="search-suggestion box-shadow bg-white" v-if="
                              data.search_result &&
                              data.search_result.length > 0
                            ">
                              <ul class="list-unstyled mb-0">
                                <li class="d-block text-left suggestion_list"
                                  v-for="(product, index) in data.search_result" :key="index" @click.prevent="
                          ...
                          <span v-if="data.image" class="product-img compare-image d-block mb-2"><img :src="data.image"
                              :alt="data.name" />
                          </span>
                          ...
                          <div v-if="data.rating">
                            <div class="star-rating">
                              <div class="product-rating-wrapper">
                                <i :data-star="data.rating" :title="data.rating"></i>
                              </div>
                            </div>
                          </div>
                          <div v-if="data.btn">
          ...
          <div class="bg-light col-lg-4 col-12 offset-lg-4 p-4" v-else>
            ...
            <div class="input-group mb-20">
              <input list="product-list-1" v-bind:placeholder="$t('Search Product')"
                class="bg-white h-100 rounded theme-input-style" v-model="search_key_1"
              ...
              ">
                <ul class="list-unstyled mb-0 first-item-search-result">
                  <li class="d-block text-left suggestion_list" v-for="(product, index) in products_suggestions_first"
                    :key="index" @click.prevent="setCompareItem(product, 'first')">
            ...
            <!--Second Item -->
            <div class="input-group mb-20">
              <input list="product-list-2" v-bind:placeholder="$t('Search Product')"
                class="bg-white h-100 rounded theme-input-style" v-model="search_key_2"
              ...
              ">
                <ul class="list-unstyled mb-0 first-item-search-result">
                  <li class="d-block text-left suggestion_list" v-for="(product, index) in products_suggestions_last"
                    :key="index" @click.prevent="setCompareItem(product, 'last')">
                    <single-product :item="product" compareSearch />
                  </li>
                </ul>
              </div>
...
.search-suggestion .suggestion_list {
  transition: 0.4s;
}

.search-suggestion .suggestion_list:hover {
  color: $c1;
}
</style>
