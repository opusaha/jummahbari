@php
    ...
    // Checking child comment thread level
    if (get_setting('thread_comments') == 1) {
        $nested_level = get_setting('thread_comments_level');
    } else {
        $nested_level = 6;
    }
@endphp
...
@foreach ($child_comment as $child)
    @php
        // getting the comment author image
        $author = null;
        ...
        <li class="comment single-comment-wrapper row" id="comment-{{ $child->id }}">
            ...
            <article class="single-comment media">
                ...
                <div class="comment-author-image">
                    ...
                    @else
                        @if (get_setting('show_avatars') == 1)
                            {{ asset('/public/comment-author-image/' . get_setting('avatar_default') . '.png') }}
                        @else
                ...
                <div class="comment-content media-body">
                    <div class="d-flex align-items-center">
                        ...
                        <span class="commented-on">
                            <time datetime="2012-09-03T10:18:04+00:00">
                                {{ date('d M Y ', strtotime($child->comment_date)) }}
                            </time>
                        </span>
                    </div>
                    ...
                    @if (!(get_setting('close_comments_for_old_blogs') == 1 && $comment_close == true))
                        @if (isset($author_name))
