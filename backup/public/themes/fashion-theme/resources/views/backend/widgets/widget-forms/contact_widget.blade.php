...
<form action="#" class=" widget_input_field_form px-3 py-3 bg-white"
    ...
    <div class="form-group">
        ...
        <textarea id="address" name="address" class="theme-input-style style--two"
            placeholder="{{ translate('Enter Address') }}">{{ $address }}</textarea>
    </div>
    <div class="@if (!empty($lang) && $lang != getdefaultlang()) area-disabled @endif">
        <div class="form-group">
            <label for="mail" class="">{{ translate('Mail') }}</label>
            <input type="text" class="form-control" id="mail" name="mail"
                placeholder="{{ translate('Enter Mail') }}" value="{{ $mail }}"
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="mobile" name="mobile"
                placeholder="{{ translate('Enter Mobile') }}" value="{{ $mobile }}"
                @if (!empty($lang) && $lang != getdefaultlang()) disabled @endif>
        </div>
    </div>
