...
class BlogController extends Controller
{
    ...
    public function loadBlogComment(Request $request)
    {
        try {
            ...
            if (get_setting('close_comments_for_old_blogs') != null && get_setting('close_comments_for_old_blogs') == 1 && $should_auto_close_comment) {
                $comment_settings['should_auto_close_comment'] = false;
            }

            $comment_settings['comment_registration'] = get_setting('comment_registration');
            $comment_settings['default_comment_status'] = get_setting('default_comment_status');
            $comment_settings['close_comments_for_old_blogs'] = get_setting('close_comments_for_old_blogs');
            $comment_settings['page_comments'] = get_setting('page_comments');
            $comment_settings['thread_comments'] = get_setting('thread_comments');
            $comment_settings['comment_order'] = get_setting('comment_order');
            $comment_settings['comments_notify_email'] = get_setting('comments_notify_email');
            $comment_settings['comments_moderation_notify_email'] = get_setting('comments_moderation_notify_email');
            $comment_settings['comment_moderation'] = get_setting('comment_moderation');
            $comment_settings['comment_previously_approved'] = get_setting('comment_previously_approved');
            $comment_settings['show_avatars'] = get_setting('show_avatars');
            $comment_settings['show_gravatars'] = get_setting('show_gravatars');
            $comment_settings['comment_max_links'] = get_setting('comment_max_links');
            $comment_settings['comment_max_images'] = get_setting('comment_max_images');
            $comment_settings['comment_moderation_keys'] = get_setting('comment_moderation_keys');
            $comment_settings['comment_disallowed_keys'] = get_setting('comment_disallowed_keys');
    ...
    public function createBlogComment(Request $request)
    {
        try {
            ...
            if (!Auth::check() && !$request->customerId) {
                if (get_setting('require_name_email') == 1) {
