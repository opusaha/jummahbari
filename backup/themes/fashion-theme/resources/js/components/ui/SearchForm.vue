<template>
  <div class="search-form-wrapper custom-search-btn-mobile">
    <!-- Search Toggle -->
    <button v-if="mobileStyle" class="p-0 bg-transparent border-0" @click="toggleSearchForm">
    ...
    <form class="search-form" :class="[
      mobileStyle
        ? 'mobile-search-form position-absolute w-100 d-flex align-items-center gap-1'
        : '',
      { active: searchFormActive },
    ]" action="#">
      <button v-if="mobileStyle" type="button"
        class="goback border-0  d-inline-flex align-items-center justify-content-center bg-light text-dark"
      ...
      <div class="input-group" :class="[
        ...
        { 'style--two p-0': styleTwo },
        { 'style--three': styleThree },
        { 'style--four': styleFour },
      ]">
        <input type="text" v-bind:placeholder="$t('Enter your search key')" class="search-input h-100"
          v-model="searching_Key" v-on:keyup="getSearchSuggestions" />
        <button v-if="!styleFour" type="submit" class="btn btn_fill custom-search-btn search-btn bg-light text-black"
