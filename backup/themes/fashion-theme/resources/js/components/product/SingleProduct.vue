<template>
  ...
  <div v-if="styleEight" class="single-product-item d-inline-block style--eight">
    <div class="position-relative overflow-hidden">
      ...
      <!-- Discount -->
      <div v-if="hasDiscount(item)" class="badge-container badge-container--style-eight">
    ...
    <!-- Summary -->
    <div class="d-flex flex-column product-summary product-summary--style-eight">
      <!-- Rating -->
      <div class="star-rating product-rating--style-eight" v-if="
        site_config.enable_product_reviews == 1 &&
        site_config.enable_product_star_rating == 1
      ">
        <div class="product-rating-wrapper">
          <i :data-star="normalizedRating(item.avg_rating)" :title="normalizedRating(item.avg_rating)"
            class="star-rating"></i>
        </div>
      </div>
      <!-- End Rating -->
      ...
      <!-- Title -->
      <h4 class="product-title product-title--style-eight">
      ...
      <!-- Price -->
      <div class="product-price-wrap" :class="{ 'has-compare-price': hasComparePrice(item) }">
        <the-currency :amount="item.base_price" tag="del" class="product-compare-price"
          v-if="!widgetSlider && hasComparePrice(item)"></the-currency>
        <span class="product-price product-current-price">
          <the-currency :amount="currentPrice(item)" tag="span"></the-currency>
        </span>
      </div>
      <!-- End Price -->
    </div>
...
export default {
  ...
  methods: {
    hasComparePrice(product) {
      return Number(product?.base_price) > Number(product?.price);
    },
    currentPrice(product) {
      return this.hasComparePrice(product)
    ...
    discountLabel(product) {
      ...
      if (!discount || Number(discount.discount_amount ?? 0) <= 0) {
        return "";
      }

      if (Number(discount.discountType) === 1) {
        return `${discount.discount_amount}% Off`;
      }

      const symbol = this.currency?.symbol ?? "";
      return `${symbol}${discount.discount_amount} Off`;
    },
    /**
     * Product quick view
     */
...
.single-product-item {
  ...
  &.style--eight {
    ...
    .product-price-wrap.has-compare-price {
      gap: 2px;
    }
