<template>
  <div class="d-flex align-items-start justify-content-between mt-10" v-if="!brandLoading && brands.length > 0">
      ...
      <ul class="list-unstyled inline-links d-flex flex-wrap mb-0">
        <li v-for="(brand, index) in brands" :key="index">
          <button class="fz-12" @click.prevent="addBrandFilter(brand)">
            <img :src="brand.logo" :alt="brand.name" v-if="brand.logo" class="mr-1" />
            {{ brand.name }}
          </button>
        </li>
      </ul>
    ...
    <!-- Load More Button -->
    <button v-if="!brandLoading && brands.length > 3" class="btn_loadmore" @click.prevent="brandBoxCollapse()">
