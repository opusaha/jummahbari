  ...
  <Dashboard>
    ...
    <template v-if="!loading">
      <div class="order_details" v-if="orderDetails != NULL">
        <div class="shadow-card mb-3 p-3" v-if="orderDetails.payment_required == enums.status.ACTIVE">
          <div class="row">
        ...
        <div class="shadow-card mb-3 p-3">
          <div class="row">
            <div class="col-12 col-lg-6">
              ...
              <p class="mb-0">
                {{ $t("Order Placed on") }} <span>{{ orderDetails.order_date }}</span>
              </p>
            </div>
            <div class="align-items-center col-12 col-lg-6 d-flex justify-content-lg-end">
              ...
              <button class="btn btn-sm fz-14 ml-10" v-if="
                  ...
                  <div class="col-md-12">
                    <div class="order-info d-flex justify-content-between">
                      ...
                      <div class="shipping-method" v-if="orderDetails.pickup_point == null">
                        {{ $t("Shipping") }}:
                        <span>
                          <the-currency :amount="product.shipping_cost" v-if="product.shipping_cost">
                          </the-currency>
                        </span>
                    ...
                    <div class="product-list-group">
                      <div class="product-list p-3 border">
                        <div class="row gy-10 align-items-center">
                          <div class="col-md-6">
                            <div class="d-flex align-items-center">
                              ...
                              <div class="item-info">
                                ...
                                <!--End Variant-->
                                <a :href="product.attachment" target="_blank" v-if="product.attachment"
                          ...
                          <div class="col-md-6">
                            <div class="align-items-center d-flex justify-content-between right-info">
                              <div class="price">
                                <the-currency :amount="product.unit_price"></the-currency>
                              ...
                              <div class="order-action-area" v-if="
                                product.delivery_status ==
                                enums.order_delivery_status.DELIVERED
                              ">
                                <!--Refund-->
                                <div class="refund-action-area">
                                  <a class="write-review" href="#" role="button" v-if="
                                    product.can_return == enums.status.ACTIVE
                                  " @click.prevent="returnProduct(product.id)">
                                    {{ $t("Return") }} / {{ $t("Refund") }}
                                  </a>
                                  <div v-else class="mb-0">
                                    <button class="write-review disable border-0 p-0 text-uppercase"
                                      title="Return not available" v-if="
                                        product.return_status.status ==
                                        enums.order_return_status.NOT_AVAILABLE
                                      " :disabled="product.return_status.status ==
                                        enums.order_return_status.NOT_AVAILABLE
                                        ">
                                      {{ $t("Return") }} / {{ $t("Refund") }}
                                    </button>
                                    <p class="text-capitalize" :class="product.return_status.class" v-else>
                                ...
                                <div class="review-action-area">
                                  <a class="write-review text-uppercase" href="#" role="button" v-if="
                              ...
                              <div class="order-action-area button" v-if="product.can_cancel == enums.status.ACTIVE">
                                <button v-if="
                                  product.can_cancel == enums.status.ACTIVE
                                " class="btn btn-sm btn_border fz-14" title="Cancel item" :disabled="product.can_cancel == enums.status.IN_ACTIVE
        ...
        <div class="row">
          <div class="col-12 col-lg-6">
            ...
            <div class="shadow-card mb-30 p-3">
              ...
              <div class="row">
                <div class="col-lg-12 mt-2 mt-lg-0" v-if="orderDetails.pickup_point == null">
                  <template v-if="orderDetails.shipping_details != null">
                    ...
                    <p>
                      ...
                      <span v-if="orderDetails.shipping_details.state != null">{{ orderDetails.shipping_details.state
                      }}, </span>
            ...
            <!--Billing address-->
            <div class="shadow-card mb-30 p-3" v-if="orderDetails.billing_details != null">
              <h5 class="order-summery-title">{{ $t("Billing Address") }}</h5>
              <div class="row">
                <div class="col-lg-12 mt-2 mt-lg-0" v-if="orderDetails.billing_details != null">
                  <p>
                    {{ orderDetails.billing_details.name }}
                  </p>
                  <p>{{ orderDetails.billing_details.phone }}</p>
                  <p>
                    <span v-if="orderDetails.billing_details.address != null">{{ orderDetails.billing_details.address
                    }}, </span>
                    <span v-if="orderDetails.billing_details.city != null">{{ orderDetails.billing_details.city
                    }}, </span>
                    <span v-if="orderDetails.billing_details.state != null">{{ orderDetails.billing_details.state
                    }}, </span>
                    <span v-if="orderDetails.billing_details.country != null">{{ orderDetails.billing_details.country
                    }}.</span>
                  </p>
                </div>
              </div>
                    ...
                    <table class="shop_table w-100">
                      <tbody>
                        <tr class="cart-subtotal">
                          <td>{{ $t("Subtotal") }}</td>
                          <td class="td-right">
                            <the-currency :amount="orderDetails.sub_total"></the-currency>
                          </td>
                        </tr>
                        <tr class="shipping-cost">
                          ...
                          <td class="td-right">
                            <span>+</span>
                            <the-currency :amount="orderDetails.total_delivery_cost"></the-currency>
                          </td>
                        </tr>
                        <tr class="order-tax border-bottom" v-if="orderDetails.total_tax > 0">
                          <td>
                            {{ $t("Tax") }}
                          </td>
                          <td class="td-right">
                        ...
                        <tr class="order-savings" v-if="orderDetails.total_discount > 0">
                          <td>
                            {{ $t("Discount") }}
                          </td>
                          <td class="td-right">
    ...
    <CModal :visible="visibleProductReviewModal" size="lg" @close="
      () => {
    ...
    ">
          ...
          <div class="form-group col-12">
            ...
            <div class="stars-wrapper">
              <ul class="stars list-unstyled mb-0">
                <li class="d-inline-block" v-for="star in 5" :key="star"
                  :class="[`star star-${star}`, { active: star <= rating }]" @click="rate(star)">
            ...
            <template v-if="errors.rating">
              <p class="fz-12 text-danger mt-1" v-for="(error, index) in errors.rating" :key="index">
                {{ error }}
              </p>
            </template>
          </div>
        ...
        <div class="row mb-20">
          <div class="form-group col-12">
            ...
            <template v-if="errors.review">
              <p class="fz-12 text-danger mt-1" v-for="(error, index) in errors.review" :key="index">
                {{ error }}
              </p>
            </template>
          </div>
        ...
        <div class="row mb-20">
          <div class="form-group col-12">
            ...
            <template v-if="errors.review_images">
              <p class="fz-12 text-danger mt-1" v-for="(error, index) in errors.review_images" :key="index">
                {{ error }}
              </p>
            </template>
          </div>
      ...
      <CModalFooter>
        <button class="btn btn_fill" @click.prevent="submitProductReview" :disabled="review_submitting">
    ...
    <CModal :visible="visibleProductReturnModal" size="lg" @close="
      () => {
    ...
    ">
      ...
      <CModalBody>
                ...
                <div class="row gy-10">
                  <div class="col-md-6">
                    <div class="product-info d-flex">
                      <div class="image">
                        <img :src="returnItem.image" :alt="returnItem.name" class="cart-image" />
                      </div>
                      <div class="title">
                        <h5>
                          {{ returnItem.name }}
                        </h5>
                        <product-variant v-if="returnItem.variant" tag="p" class="mb-0"
                          :variant="returnItem.variant"></product-variant>
                      </div>
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="right-info d-flex justify-content-between">
                      <div class="price">
                        <the-currency :amount="returnItem.unit_price"></the-currency>
                      </div>
                      <div class="quantity">
                      ...
                      <div class="quantity">
                        {{ $t("Total") }}:<the-currency
                          :amount="returnItem.unit_price * returnItem.quantity"></the-currency>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div class="row mb-20">
          <div class="form-group col-12">
            <label class="font-weight-bold fz-12 mb-2">
              {{ $t("Refund Reason") }}
              <span class="text-danger">*</span></label>
            <select class="theme-input-style p-0" v-model="refund_reason"
              v-bind:placeholder="$t('Select a refund reason')">
              <option v-for="(reason, index) in all_refund_reasons" :key="index" :value="reason.id">
                {{ reason.name }}
              </option>
            </select>
            <template v-if="errors.refund_reason">
              <p class="fz-12 text-danger mt-1" v-for="(error, index) in errors.refund_reason" :key="index">
                {{ error }}
              </p>
            </template>
          </div>
        ...
        <div class="row mb-20">
          <div class="form-group col-12">
            ...
            <template v-if="errors.refund_comment">
              <p class="fz-12 text-danger mt-1" v-for="(error, index) in errors.refund_comment" :key="index">
                {{ error }}
              </p>
            </template>
          </div>
        ...
        <div class="row mb-20">
          <div class="form-group col-12">
            ...
            <base-file-input id="reviewImages" name="reviewImages" v-on:getFileInput="returnImages($event)" />
            <template v-if="errors.return_images">
              <p class="fz-12 text-danger mt-1" v-for="(error, index) in errors.return_images" :key="index">
                {{ error }}
              </p>
            </template>
          </div>
      ...
      <CModalFooter>
        <button class="btn btn_fill" @click.prevent="submitProductReturn" :disabled="return_submitting">
    ...
    <!--Order cancel modal-->
    <CModal :visible="visibleOrderCancelModal" size="sm" @close="
      () => {
    ...
    ">
      ...
      <CModalBody>
        <div class="text-center">
          ...
          <button class="btn btn_fill mt-20 rounded-1" @click.prevent="confirmCancelOrder">
            {{ $t("Confirm") }}
          </button>
        </div>
      </CModalBody>
