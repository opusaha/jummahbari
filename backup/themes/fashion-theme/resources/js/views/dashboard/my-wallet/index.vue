  ...
  <Dashboard>
    <div class="purchase_history">
      ...
      <template v-if="!loading">
        ...
        <div class="row">
          <div class="col-lg-4">
            <dashboard-card title="Available Balance" currency :counter="wallet_available_balance"></dashboard-card>
          </div>
          <div class="col-lg-4">
            <dashboard-card title="Pending Balance" currency :counter="wallet_pending_balance"></dashboard-card>
          </div>
          <div class="col-lg-4">
            <div class="single-overview white-box pt-15 pb-15 pl-20 pr-20 mb-30 text-center">
              <h2 class="font-weight-medium mb-0">
                <button class="wallet-recharge btn btn-circle" @click="
                  () => {
        ...
        <div v-if="tableData.length > 0">
                ...
                <CTable class="style1 mb-0" :class="tableData.length == 1 ? 't-mh-160' : ''">
                  ...
                  <CTableBody>
                    <CTableRow v-for="(tdata, index) in tableData" :key="index">
                      ...
                      <CTableDataCell>
                        <the-currency :amount="tdata.recharge_amount"></the-currency>
                      </CTableDataCell>
                      <CTableDataCell>
                        <p class="text-capitalize d-flex align-items-center gap-1" v-if="tdata.type == 'credited'">
                          <span class="material-icons text-success">
                            arrow_upward
                          </span>
                          <span>{{ tdata.type }}</span>
                        </p>
                        <p class="text-capitalize d-flex align-items-center gap-1" v-else>
                      ...
                      <CTableDataCell>
                        <p :class="`status-btn badge text-capitalize ${tdata.status}`">
                          {{ tdata.status }}
                        </p>
                      </CTableDataCell>
                    </CTableRow>
          ...
          <div class="row align-items-center mt-30" v-if="tableData.length > 0">
            <div class="col-md-6">
              <!-- Showing Per Page -->
              <showing-per-page class="text-center text-md-start" :items-per-page="perPage" :total-items="totalRows"
    ...
    <CModal :visible="visibleWalletRechargeModal" size="md" @close="
      () => {
    ...
    ">
        ...
        <div class="row">
          ...
          <div class="col-12">
            <ul class="list-unstyled form-selector-list mb-3">
              <li class="p-1 single-form-selector w-50 m-0" v-if="siteSettings.enable_online_recharge == status.ACTIVE">
                <span class="custom-radio-btn">
                  <label class="mb-0 p-2">
                    <input type="radio" class="wallet-recharge-type" name="wallet-recharge-type" value="1"
              ...
              <li class="p-1 single-form-selector w-50 m-0"
                v-if="siteSettings.enable_offline_recharge == status.ACTIVE">
                <span class="custom-radio-btn">
                  <label class="mb-0 p-2">
                    <input type="radio" class="wallet-recharge-type" name="wallet-recharge-type" value="2"
                      v-model="selected_recharge_type" />
                    <span class="label-title">
                      {{ $t("Offline Recharge") }}
                    </span>
          ...
          ">
            <div class="row mb-20">
              <div class="form-group col-12 mb-20">
                ...
                <select class="theme-input-style" name="payment_method" v-model="selected_online_payment_method">
                  <option v-for="(method, index) in online_payment_methods" :key="index" :value="method.id">
                    {{ method.name }}
                  </option>
                </select>
                <template v-if="errors.payment_method">
                  <p class="fz-12 text-danger mt-1" v-for="(error, index) in errors.payment_method" :key="index">
                    {{ error }}
                  </p>
                </template>
              </div>
              <div class="form-group col-12">
                <label class="font-weight-bold">
                  {{ $t("Recharge amount") }}
                  <span class="text-danger">*</span></label>
                <input type="text" class="theme-input-style" v-model="recharge_amount" />
                <template v-if="errors.recharge_amount">
                  <p class="fz-12 text-danger mt-1" v-for="(error, index) in errors.recharge_amount" :key="index">
                    {{ error }}
                  </p>
                </template>
              </div>
          ...
          <!--End online recharge-->
          <!--Offline recharge-->
          <div class="col-12" v-if="selected_recharge_type == recharge_type.OFFLINE">
            <div class="row mb-20">
              ...
              <div class="form-group col-12">
                ...
                <ul class="list-unstyled form-selector-list mb-3">
                  <li class="single-form-selector" v-for="(payment, index) in offline_payment_methods" :key="index">
                    <span class="custom-radio-btn">
                      <label>
              ...
              <div class="bg-light col-12 py-2 mb-20" v-if="selected_offline_payment != null">
                <div v-html="selected_offline_payment.instruction" v-if="selected_offline_payment.instruction != null">
                </div>
              ...
              <div class="form-group col-12 mb-20">
                <label class="font-weight-bold">
                  {{ $t("Recharge amount") }}
                  <span class="text-danger">*</span></label>
                <input type="text" class="theme-input-style" v-model="recharge_amount" placeholder="0.00" />
                <template v-if="errors.recharge_amount">
                  <p class="fz-12 text-danger mt-1" v-for="(error, index) in errors.recharge_amount" :key="index">
                    {{ error }}
                  </p>
              ...
              <div class="form-group col-12 mb-20">
                <label class="font-weight-bold">
                  {{ $t("Transaction ID") }}
                  <span class="text-danger">*</span></label>
                <input type="text" class="theme-input-style" v-model="transaction_id"
                  v-bind:placeholder="$t('Enter transaction id')" />
                <template v-if="errors.transaction_id">
                  <p class="fz-12 text-danger mt-1" v-for="(error, index) in errors.transaction_id" :key="index">
                    {{ error }}
                  </p>
                </template>
              </div>
              ...
              <div class="form-group col-12 mb-20">
                ...
                <base-file-input id="transaction_image" name="transaction_image"
                  v-on:getFileInput="transactionImage($event)" single />
                <template v-if="errors.transaction_image">
                  <p class="fz-12 text-danger mt-1" v-for="(error, index) in errors.transaction_image" :key="index">
                    {{ error }}
                  </p>
                </template>
              </div>
      ...
      <CModalFooter>
        <button class="btn btn_fill" @click.prevent="submitWalletRecharge" :disabled="wallet_recharge_submitting">
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.query.recharge) {
      ...
      if (this.$route.query.recharge == "FAILED") {
        this.$toast.error(this.$t("Wallet recharge failed"));
      }

      if (this.$route.query.recharge == "CANCELLED") {
        this.$toast.error(this.$t("Wallet recharge cancelled"));
      }
  ...
  methods: {
    ...
    getCustomerWalletSummary() {
      axios
        ...
        .then((response) => {
          if (response.data.success) {
            this.wallet_available_balance = response.data.summary.total_available;
    ...
    submitWalletRecharge() {
      ...
      if (this.selected_recharge_type == this.recharge_type.ONLINE) {
        ...
        formData.append("payment_method", this.selected_online_payment_method);
