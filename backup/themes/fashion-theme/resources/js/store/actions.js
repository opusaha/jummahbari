 ...
 */
    ...
    if (this.state.isCustomerLogin) {
      ...
      return axios.post('/api/v1/ecommerce-core/customer/cart/store-cart-item', formData, {
        ...
        if (response.data.success) {
          context.commit("storeCartItems", { item: data.item, messages: data.messages });
        }
        if (!response.data.success) {
          $toast.error(data.messages.limit_error);
        }
      }).catch((error) => {
        context.commit("showPreloader", false);
  ...
  */
   ...
   */
  storePickupPointCheckout(context, data) {
    context.commit("storePickupPointCheckout", data);
  },
  /**
  * Store pickup point details
  */
  storePickupPoint(context, data) {
    context.commit("storePickupPoint", data);
  },
  /**
  ...
  setFinalSubtotal(context, data) {
    context.commit("setFinalSubtotal", data);
  },
  /**
   * Store order tax
   */
